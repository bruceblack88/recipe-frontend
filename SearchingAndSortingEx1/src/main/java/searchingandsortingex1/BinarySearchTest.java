 ...
 * June 13, 2021
 */
package searchingandsortingex1;
...
public class BinarySearchTest {
    ...
    public static void main(String[] args) {
        ...
        while (searchInt != -1) {
            ...
            // search key enter
            System.out.printf("Please enter a key to be searched by a recursive approach: ");
            int n = input.nextInt();
            // recursive function
            int index = recursiveSearchBlack(data, n, 0, data.length);
    ...
    private static int recursiveSearchBlack(int[] data, int key, int start, int end) {

        if (start < end) {
            int mid = start + (end - start) / 2;
            if (key < data[mid]) {
                return recursiveSearchBlack(data, key, start, mid);

            } else if (key > data[mid]) {
                return recursiveSearchBlack(data, key, mid + 1, end);
